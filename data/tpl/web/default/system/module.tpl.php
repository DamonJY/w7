			...
			<tr ng-repeat="module in module_list">
				...
				<td class="text-left">
					<div class="link-group">
						...
						<a href="<?php  echo url('system/module/uninstall')?>&name={{ module.name }}" class="del" ng-if="isFounder == 1" onclick="return confirm('确认要停用模块吗？')">停用</a>
...
<div class="js-system-module-detail" ng-controller="detailCtrl" ng-cloak>
	...
	<div class="btn-group we7-btn-group we7-margin-bottom">
		...
		<a href="javascript:;" ng-click="changeShow('subscribe')" class="btn " ng-class="{'active' : show == 'subscribe'}">公众平台消息处理选项</a>
		<?php  } ?>
	...
	<table class="table we7-table table-hover vertical-middle" ng-show="show == 'upgrade'" ng-if="isFounder == 1 && upgradeInfo.from == 'cloud'" ng-repeat="branch in upgradeInfo.branches">
		...
		<tr ng-if="branch.displayorder >= upgradeInfo.site_branch.displayorder">
			...
			<td class="text-right">
				...
				<a ng-href="{{ './index.php?c=cloud&a=process&m='+upgradeInfo.name+'&is_upgrade=1' }}" onclick="return confirm('确定要升级到此分之的最新版吗？')" ng-if="branch.id == upgradeInfo.site_branch.id && branch.version.version !=  moduleinfo.version" class="btn btn-primary">升级</a>
		...
		<tr>
			...
			<td colspan="4" class="text-right">
				<a class="color-default view-detail" ng-if="branch.id == upgradeInfo.site_branch.id && branch.version.version !=  moduleinfo.version" data-toggle="collapse" href="#version-detail-{{ branch.id }}" onclick="change($(this))">查看详情 <i class="wi wi-angle-down"></i></a>
		...
		<tr class="collapse" id="version-detail-{{ branch.id }}">
