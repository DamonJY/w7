				...
				if (angular.isDefined(val)) {
					...
					for (var i = 0; i < editmedia.length; i++) {
						if ($(editmedia.get(i)).find('.edit-gray').data('media') == val) {
							...
							angular.forEach(inputValArr, function(val, key) {
								inputValArr[key] = htmlDecode(val);
							});
							replyVal = inputValArr.join(',');
							$(':hidden[name="reply[reply_'+type+']"]').val(replyVal);
							$(editmedia.get(i)).find('.edit-gray').data('media', htmlEncode(material.content));
							$(editmedia.get(i)).html(
								...
								'	</div>'+
								'	<div class="opr">'+
								'		<a href="javascript:;" data-media="'+htmlEncode(material.content)+'" class="edit-gray" onclick="select_mediaid(\'basic\', \''+htmlEncode(material.content)+'\')">编辑</a>'+
								'		<a href="javascript:;" data-media="'+htmlEncode(material.content)+'" class="del-gray" onclick="delmedia(\''+type+'\', this)">删除</a>'+
								'	</div>'
							);
							break;
						}
				...
				case 'basic':
					angular.forEach(val, function(basicVal, basicKey) {
						...
						$('.keywords-list').append(
							...
							'	</div>'+
							'	<div class="opr">'+
							'		<a href="javascript:;" data-media="'+htmlEncode(basicVal.content)+'" class="edit-gray" onclick="select_mediaid(\'basic\', \''+htmlEncode(basicVal.content)+'\')">编辑</a>'+
							'		<a href="javascript:;" data-media="'+htmlEncode(basicVal.content)+'" class="del-gray" onclick="delmedia(\''+type+'\', this)">删除</a>'+
							'	</div>'+
							'</div>'+
							'</li>'
						);
